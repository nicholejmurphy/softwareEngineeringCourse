...
class Boggle():
    ...
    def check_valid_word(self, board, word):
        ...
        not_duplicate_word = self.check_if_not_duplicate_word(word)

        if word_exists and valid_word:
            if not_duplicate_word:
                result = "ok"
            else:
                result = f"You already guessed {word.capitalize()}!"
        elif word_exists and not valid_word:
            result = f"{word.capitalize()} is not on board!"
        else:
            result = f"{word.capitalize()} is not a valid word!"

        return result

    def check_if_not_duplicate_word(self, word):
        """Check if this word has already been guessed, if not, add to list of guesses"""
        guessedWords = session['guessed-words']
        if guessedWords.count(word) == 0:
            guessedWords.append(word)
            session['guessed-words'] = guessedWords
            return True
        else:
            return False
